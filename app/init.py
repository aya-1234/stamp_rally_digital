...
class Login(db.Model):
    ...
    loginId = db.Column(db.String(20), unique=True, nullable=False)
    isLoggedin = db.Column(db.Boolean, nullable=False)
    isAgree = db.Column(db.Boolean, nullable=False)
    isEnded = db.Column(db.Boolean, nullable=False)
...
class Quiz_Response(db.Model):
    ...
    isCorrected = db.Column(db.Boolean, nullable=False)
...
class Survey_Select(db.Model):
    __tablename__ = 'SURVEY_SELECT'
    id = db.Column(db.Integer, primary_key=True, autoincrement=True, nullable=False)
    survey_id = db.Column(db.Integer, db.ForeignKey('SURVEY.id'), nullable=False)
    select = db.Column(db.String(50), nullable=False)
...
def initialize_db():
    with app.app_context():  # アプリケーションコンテキストを作成
        ...
        if db.session.query(Checkpoint).count() == 0:
            ...
            test_logins = [
                Login(isUsed=False, loginId="test_user1", isLoggedin=False, isAgree=False, isEnded=False, issuedAt=datetime(2022, 1, 1)),
                Login(isUsed=False, loginId="test_user2", isLoggedin=False, isAgree=False, isEnded=False, issuedAt=datetime(2022, 1, 2)),
                Login(isUsed=False, loginId="test_user3", isLoggedin=False, isAgree=False, isEnded=False, issuedAt=datetime(2022, 1, 3))
            ]
            ...
            test_survey_selects = [
                Survey_Select(survey_id=test_surveys[0].id, select="選択肢1", value=1),
                Survey_Select(survey_id=test_surveys[0].id, select="選択肢2", value=2),
                Survey_Select(survey_id=test_surveys[1].id, select="選択肢1", value=1),
                Survey_Select(survey_id=test_surveys[1].id, select="選択肢2", value=2 )
            ]
            db.session.add_all(test_surveys)
            db.session.flush()  

            test_stamps = [
                Stamp(login_id=test_logins[0].id, checkpoint_id=test_checkpoints[0].id, created=datetime(2022, 1, 1)),
                Stamp(login_id=test_logins[1].id, checkpoint_id=test_checkpoints[1].id, created=datetime(2022, 1, 2))
            ]
